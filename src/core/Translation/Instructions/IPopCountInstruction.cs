...
public sealed class IPopCountInstruction : Instruction
{
    ...
    internal IPopCountInstruction(BasicBlock block, Variable result, Variable operand)
        : base(block)
    {
        Check.Null(result);
    ...
    {
        return $"{Result} = ipopcount {Operand}";
    }
}
