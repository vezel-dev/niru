...
public sealed class FTruncateInstruction : Instruction
{
    ...
    internal FTruncateInstruction(BasicBlock block, Variable result, Variable operand)
        : base(block)
    {
        Check.Null(result);
        Check.Argument((result.Unit, result.Type) == (block.Unit, TypeId.Float32), result);
        Check.Null(operand);
        Check.Argument((operand.Unit, operand.Type) == (block.Unit, TypeId.Float64), operand);
