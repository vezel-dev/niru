...
public sealed class ICompareInstruction : Instruction
{
    ...
    internal ICompareInstruction(
        BasicBlock block, IntegerComparison comparison, Variable result, Variable left, Variable right)
        : base(block)
    {
    ...
    {
        return $"{Result} = icmp {Comparison.ToIRString()} {Left}, {Right}";
    }
}
