...
public sealed class UExtendInstruction : Instruction
{
    ...
    internal UExtendInstruction(BasicBlock block, Variable result, Variable operand)
        : base(block)
    {
        Check.Null(result);
        Check.Argument((result.Unit, result.Type) == (block.Unit, TypeId.Int64), result);
        Check.Null(operand);
        Check.Argument((operand.Unit, operand.Type) == (block.Unit, TypeId.Int32), operand);
