...
public sealed class ILeadingZeroCountInstruction : Instruction
{
    ...
    internal ILeadingZeroCountInstruction(BasicBlock block, Variable result, Variable value, Variable fallback)
        : base(block)
    {
        Check.Null(result);
    ...
    {
        return $"{Result} = ilzcount {Value}, {Fallback}";
    }
}
